<script lang="ts">
  ...
  function handleEdit(event: MouseEvent, id: string) {
    anchor = {
      getBoundingClientRect: () => (event.currentTarget as HTMLElement).getBoundingClientRect(),
    };
    selectedRow = id;
  }
</script>
...
<div class="w-full">
  ...
  <Popover {open} onOpenChange={(nextOpen) => !nextOpen && (selectedRow = undefined)}>
    <PopoverContent side="left" {anchor}>
      ...
      <PopoverDescription>The popover anchors to the selected action button.</PopoverDescription>
      <div class="mt-3">
        <PopoverClose>
          {#snippet child({ props })}
